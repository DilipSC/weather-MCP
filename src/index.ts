...
server.tool(
    ...
    async({state})=>{
        ...
        const alertData = await makeWeatherRequest<AlertsResponse>(alertsUrl)
...
server.tool(
  ...
  async({latitude,longitude})=>{
    ...
    const pointsData=await makeWeatherRequest<PointsResponse>(pointsUrl)
    ...
    const forecastData=await makeWeatherRequest<ForecastResponse>(forecastUrl)
    if(!forecastData){
      return{
        content:[
          {
